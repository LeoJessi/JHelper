 ...
 * - 多 CDN / 多域名调度
 */
object NetLatencyTester {

    /**
     * 单个源测速结果
     *
     * @param source 对应的播放源
     * @param latency 延迟耗时（毫秒）
     *                Long.MAX_VALUE 表示测试失败（不可用）
     */
    data class TestResult(val source: String, val latency: Long)
    ...
    fun selectBestNet(sources: List<String>, callback: (SelectionResult) -> Unit) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val results = sources.map { source ->
                async {
                    val latency = testLatency(source)
                    TestResult(source, latency)
                }
            }.awaitAll()
     ...
     */
    @JvmStatic
    fun testSingleLatency(url: String): Long = testLatency(url)

    private fun testLatency(url: String): Long {
        val uri = URL(url)
        val host = uri.host
